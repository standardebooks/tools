	...
	for root, directories, filenames in os.walk(self.path):
		...
		for filename in natsorted(filenames):
			...
			if filename.suffix == ".xhtml":
				...
				nodes = dom.xpath("/html/body//text()[re:test(., '\\b(a (priori|posteriori|fortiori)|(?<!reductio )ad (hominem|absurdum|nauseam|infinitum|interim|valem)|in (loco|situ|vitro|absentia|camera|statu quo)|in toto[^’]|more suo)\\b', 'i') and not(ancestor-or-self::*[@data-css-font-style='italic']) and not(parent::*[re:test(name(), '^h[1-6]$') and @xml:lang='la']) and not(ancestor::nav[contains(@epub:type, 'toc')]) ]")
				if nodes:
				...
				matches = regex.findall(r"[,;]{2,}.{0,20}", file_contents.replace("&amp;", "")) + regex.findall(r"(?:“\s*“|”\s*”|’ ’|‘\s*‘).{0,20}", file_contents) +  regex.findall(r"[\p{Letter}][,\.:;]\s[,\.:;]\s?[\p{Letter}<].{0,20}", file_contents, flags=regex.IGNORECASE)
				if matches:
