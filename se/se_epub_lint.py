...
def _lint_metadata_checks(self) -> list:
	...
	if nodes:
		messages.append(LintMessage("m-009", f"[xml]<meta property=\"se:url.vcs.github\">[/] value does not match expected: [url]{self.generated_github_repo_url}[/].", se.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		...
		if sorted(node_string_values) != node_string_values:
			messages.append(LintMessage("m-053", "[xml]<meta property=\"se:subject\">[/] elements not in alphabetical order.", se.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage("", nodes[0].sourceline)]))
...
def _lint_special_file_checks(self, source_file: SourceFile, dom: se.easy_xml.EasyXmlTree, ebook_flags: dict, special_file: str) -> list:
  ...
  self
	...
	if self.is_se_ebook and special_file == "colophon":
		...
		if nodes:
			messages.append(LintMessage("s-105", "Dates in the colophon need to be wrapped in an [xhtml]<time>[/] element.", se.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
...
def _lint_xhtml_syntax_checks(self, source_file: SourceFile, dom: se.easy_xml.EasyXmlTree, ebook_flags: dict, language: str, section_tree: list[EbookSection]) -> list:
	...
	if nodes:
		messages.append(LintMessage("s-055", "[xhtml]<th>[/] element not in [xhtml]<thead>[/] ancestor. Note: [xhtml]<th>[/] elements used as mid-table headings or horizontal row headings require the [attr]scope[/] attribute.", se.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-096", "Heading element in half title page missing the [val]fulltitle[/] semantic.", se.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
...
def _lint_xhtml_typography_checks(source_file: SourceFile, dom: se.easy_xml.EasyXmlTree, special_file: str | None, ebook_flags: dict, missing_files: list, self) -> tuple:
	...
	if matches:
		messages.append(LintMessage("t-046", "[text]῾[/] (U+1FFE) detected. Use [text]ʽ[/] (U+02BD) instead.", se.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(matches)))
	...
	matches = source_file.findall("´")
	if matches:
		messages.append(LintMessage("t-055", "Lone acute accent ([val]´[/]). A more accurate Unicode character like prime for coordinates or measurements, or combining accent or breathing mark for Greek text, is required.", se.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(matches)))
	...
	for root, directories, filenames in os.walk(self.path):
		...
		for filename in natsorted(filenames):
			...
			if filename.suffix == ".xhtml":
				...
				messages += _lint_xhtml_xhtml_checks(source_file, dom, local_css_path)
