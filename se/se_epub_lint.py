...
"m-044", f"The spine order does not match the order of the ToC and landmarks. Expected [text]{node.attribute('idref')}[/], found [text]{toc_files[index]}[/]."
	...
	for root, directories, filenames in os.walk(self.path):
		...
		for filename in natsorted(filenames):
			...
			if filename.suffix == ".xhtml":
				...
				nodes = dom.xpath("/html/body//*[re:test(name(), '^h[1-6]$')][./text()[re:test(., '^(Part|Book|Volume|Section)')] or (./span[contains(@epub:type, 'ordinal')] and not(./span[contains(@epub:type, 'label')]))]")
				if nodes:
