"""
british2american command
"""

import argparse

import se.typography
from se.common import get_target_filenames, print_error, print_warning


def cmd_british2american() -> int:
	"""
			...
			with open(filename, "r+", encoding="utf-8") as file:
				...
				if not args.force:
					if se.typography.guess_quoting_style(xhtml) == "american":
						...
						if args.verbose:
							print("")
						print_warning("File appears to already use American quote style, ignoring. Use --force to convert anyway.{}".format(" File: " + str(filename) if not args.verbose else ""), args.verbose)
