...
def cmd_clean() -> int:
	...
	for filename in get_target_filenames(args.targets, (".xhtml", ".svg", ".opf", ".ncx", ".xml"), []):
		...
		try:
			...
			print_error(str(ex) + f" File: {filename}", args.verbose)
			return ex.code

		if args.verbose:
			print(" OK")

	for filename in get_target_filenames(args.targets, (".css",), []):
