...
import se
from se.common import get_firefox_path, print_error


def cmd_compare_versions() -> int:
	"""
	...
	try:
		...
		print_error("Couldn’t locate firefox. Is it installed?")
	...
	if which_compare:
		...
		print_error("Couldn’t locate compare. Is imagemagick installed?")
		return se.MissingDependencyException.code
	...
	if "firefox" in (p.name() for p in psutil.process_iter()):
		print_error("Firefox is required, but it’s currently running. Stop all instances of Firefox and try again.")
	...
	for target in args.targets:
		...
		if "nothing to commit" in git_command.status():
			print_error("Repo is clean. This script must be run on a dirty repo.", args.verbose)
			continue
