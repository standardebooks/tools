...
complete -c se -n "__fish_se_no_subcommand" -a print-manifest-and-spine -d "Print <manifest> and <spine> tags to standard output for the given Standard Ebooks source directory"
complete -c se -A -n "__fish_seen_subcommand_from print-manifest-and-spine" -s h -l help -x -d "show this help message and exit"
complete -c se -A -n "__fish_seen_subcommand_from print-manifest-and-spine" -s m -l manifest -d "only print the manifest"
