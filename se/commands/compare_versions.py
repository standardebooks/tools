...
def _screenshot_html_element(driver: 'WebDriver', filename: Path, output_path: Path) -> None:
	"""
	Load an XHTML file and save a screenshot of its full `<html>` element.
	"""

	driver.get(filename.as_uri())
	...
	"""
	driver.execute_async_script(wait_for_layout_script) # pyright: ignore[reportUnknownMemberType] Broken Selenium type hint here.

	capabilities = cast(dict[str, object], driver.capabilities) # pyright: ignore[reportUnknownMemberType] Broken Selenium type hint here.
	browser_type = se.browser.BrowserType.from_string(str(capabilities.get("browserName", "")).lower())

	# Chrome only screenshots the viewport, even when we specify the entire `<html>` element, so use special logic for Chrome.
	if browser_type == se.browser.BrowserType.CHROME:
		_screenshot_chrome_full_page(driver, output_path)

	elif browser_type == se.browser.BrowserType.FIREFOX:
		try:
			driver.find_element("tag name", "html").screenshot(str(output_path)) # pyright: ignore Broken Selenium type hint here.
			...
			_screenshot_firefox_full_page(driver, output_path)

	else:
		raise se.MissingDependencyException("Couldn't initialize default web browser.")

def _screenshot_firefox_full_page(driver: 'WebDriver', output_path: Path) -> None:
	"""
	...
	"""
	dimensions = cast(dict[str, int], driver.execute_script(get_dimensions_script)) # pyright: ignore[reportUnknownMemberType] Broken Selenium type hint here.
	full_page_image = None

	for scroll_position in range(0, dimensions["height"], dimensions["viewportHeight"]):
		actual_scroll_position = cast(int, driver.execute_script("window.scrollTo(0, arguments[0]); return window.scrollY;", scroll_position)) # pyright: ignore[reportUnknownMemberType] Broken Selenium type hint here.

		with Image.open(BytesIO(driver.get_screenshot_as_png())) as viewport_image: # pyright: ignore[reportUnknownMemberType] Broken Selenium type hint here.
...
def _screenshot_chrome_full_page(driver: 'WebDriver', output_path: Path) -> None:
	"""
	...
	result = cast(dict[str, str], driver.execute_cdp_cmd("Page.captureScreenshot", screenshot_options)) # pyright: ignore[reportUnknownMemberType] Broken Selenium type hint here.
...
def compare_versions(plain_output: bool) -> int:
	...
	driver = None
	try:
		driver = se.browser.initialize_selenium_webdriver()
