	...
	try:
		...
		for filepath in se.get_target_filenames(args.targets, ".xhtml"):
			...
			while pad:
				...
				if char in (curses.KEY_NPAGE, 523):
					if pad_height - pad_y - screen_height > 0:
				...
				if char in (curses.KEY_PPAGE, 564):
					if pad_y > 0:
