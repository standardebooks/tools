        ...
        if not val:
            for x in matches:
                ...
                if is_package_identifier:
                    from calibre.utils.standardebooks import deterministic_uuid
                    self.set_text(x, str(deterministic_uuid()))
...
class OPFCreator(Metadata):
    ...
    def __init__(self, base_path, other):
        ...
        if self.application_id is None:
            from calibre.utils.standardebooks import deterministic_uuid
            self.application_id = str(deterministic_uuid())
...
def metadata_to_opf(mi, as_string=True, default_lang=None):
    ...
    if not mi.application_id:
        from calibre.utils.standardebooks import deterministic_uuid
        mi.application_id = str(deterministic_uuid())

    if not mi.uuid:
        from calibre.utils.standardebooks import deterministic_uuid
        mi.uuid = str(deterministic_uuid())
