...
from calibre.utils.standardebooks import deterministic_asin, deterministic_mobi_uid
...
class KF8Book:
    ...
    def build_records(self, writer, for_joint):
        ...
        self.uid = deterministic_mobi_uid()
    ...
    def record0(self):
        ...
        record = MOBIHeader()(**kwargs)

        # The ASIN and CDE content key replace calibre's shorter UUID metadata while preserving record size.
        asin_size = len(deterministic_asin().encode('ascii'))
        removed_metadata_size = (8 + 36) + 12
        added_metadata_size = (8 + asin_size) * 2 + 12
        return record[:-(added_metadata_size - removed_metadata_size)]
    ...
    def write(self, outpath):
        ...
        with open(outpath, 'wb') as f:
            ...
            f.write(title)
            f.write(pack(b'>HHIIIIII', 0, 0, 0, 0, 0, 0, 0, 0))
            f.write(b'BOOKMOBI')
