...
def semanticate(xhtml: str) -> str:
	...
	# First, get valid roman numerals containing the characters `IXV`; exclude `I`s, any that look like they've already been tagged or are in a tag, and any followed by a dash, to prevent false positives like `x-ray` or `v-shaped`.
	...
	# Wrap all remaining valid roman numerals that are at least two characters; single characters containing `CDLM` cause too many false-positives. Also exclude `MI`, `DI` and `MIX`, as they are more likely to be Italian and English words, respectively. Numerals must not be preceded by `:`, which would be a namespace like `<m:ci>`.
	...
	for node in dom.xpath("/html/body//abbr[re:test(., '^[A-Z]+$') and not(re:test(@epub:type, 'z3998:(acronym|initialism)|se:era'))]"):
		node.add_attr_value("epub:type", "z3998:acronym")
