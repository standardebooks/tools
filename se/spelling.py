...
def modernize_spelling(xhtml: str) -> str:
	...
	xhtml = regex.sub(r"\bPhantas(y|ie)\b", r"Fantasy", xhtml)			# Phantasie -> Fantasy
	xhtml = regex.sub(r"\bphantas(y|ie)\b", r"fantasy", xhtml)			# phantasie -> fantasy
	...
	xhtml = regex.sub(r"\bingulf", r"engulf", xhtml)				# ingulf -> engulf
	xhtml = regex.sub(r"\bNick-?nack", r"Knickknack", xhtml)			# nicknack -> knickknack
	xhtml = regex.sub(r"\bnick-?nack", r"knickknack", xhtml)			# nicknack -> knickknack
	xhtml = regex.sub(r"\b([Bb])andanna", r"\1andana", xhtml)			# bandanna -> bandana
	...
	xhtml = regex.sub(r"\b([Bb])arytone", r"\1aritone", xhtml)			# barytone -> baritone
	xhtml = regex.sub(r"\b([Cc])urlycue", r"\1urlicue", xhtml)			# curlycue -> curlicue
	xhtml = regex.sub(r"\b([Hh])asheesh", r"\1ashish", xhtml)			# hasheesh -> hashish
	...
	xhtml = regex.sub(r"\b([Dd])[eé]b[aá]cle", r"\1ebacle", xhtml)			# débâcle -> debacle
	...
	xhtml = regex.sub(r"Shake?spea?r([^ie])", r"Shakespeare\1", xhtml)		# Shakesper/Shakspear -> Shakespeare
	xhtml = regex.sub(r"Shake?spea?re", r"Shakespeare", xhtml)			# Shakespere/Shakspeare -> Shakespeare
	xhtml = regex.sub(r"Shakspea?re?’s", r"Shakespeare’s", xhtml)			# Shakspere’s/Shakspear’s -> Shakespeare’s
	xhtml = regex.sub(r"Shakspea?r[ei]an", r"Shakespearean", xhtml)			# Shaksperean/Shaksperian -> Shakespearean
	...
	xhtml = regex.sub(r"\bKelt(s|ic|ish)?\b", r"Celt\1", xhtml)			# Kelt -> Celt
	...
	xhtml = regex.sub(r"\b([Dd])iarrhœa", r"\1iarrhoea", xhtml)
