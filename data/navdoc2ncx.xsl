		...
		<xsl:element name="ncx">
			...
			<xsl:if test="//xhtm:nav[@epub:type='lot']">
				<xsl:element name="navList">
					...
					<xsl:apply-templates select="//xhtm:nav[@epub:type='lot']/xhtm:ol" mode="flat"/>
				</xsl:element>
			</xsl:if>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="addHeading">
		...
		<xsl:if test="//xhtm:nav[@epub:type=$navType]/xhtm:*[starts-with(local-name(.),'h')]">
			<xsl:element name="navInfo">
				<xsl:element name="text">
					<xsl:value-of select="//xhtm:nav[@epub:type=$navType]/xhtm:*[starts-with(local-name(.),'h')]/node()"/>
				</xsl:element>
			</xsl:element>
		</xsl:if>
	</xsl:template>
		...
		<xsl:for-each select="xhtm:li">
			...
			<xsl:element name="pageTarget">
				<xsl:attribute name="type">
					<xsl:choose>
						<xsl:when test="@epub:type and contains('frontmatterbodymatterbackmatter', @epub:type)">
							<!-- use the @epub:type with designator if present to set the correct type -->
							<xsl:value-of select="replace(@epub:type, 'matter', '')"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
